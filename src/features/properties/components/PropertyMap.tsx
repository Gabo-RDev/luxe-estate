...
export default function PropertyMap({ location, lat, lng }: PropertyMapProps) {
	...
	// Fix for React leaflet hydration bounds issue
	...
	if (!mounted) {
		return (
			<div className="relative w-full aspect-4/3 rounded-lg bg-slate-100 flex items-center justify-center">
	...
	return (
		<div className="relative w-full aspect-4/3 rounded-lg overflow-hidden bg-slate-100 z-0">
			...
			>
				...
				/>
				{hasCoords && <Marker position={position} icon={customIcon} />}
			</MapContainer>
			<a
