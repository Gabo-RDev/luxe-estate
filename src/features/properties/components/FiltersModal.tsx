...
import {
	...
	FILTERS_DEFAULTS,
	PRICE_BOUNDS,
} from '@/features/properties/filtersConfig';
...
export default function FiltersModal({ isOpen, onClose }: FiltersModalProps) {
	...
	// --- Derived percentages for the slider ---
	const getPercentage = (price: number) => {
		const pct =
			((price - PRICE_BOUNDS.MIN) / (PRICE_BOUNDS.MAX - PRICE_BOUNDS.MIN)) *
			100;
		return Math.max(0, Math.min(100, pct));
	};

	const minPct = getPercentage(minPrice);
	const maxPct = getPercentage(maxPrice);
	...
	return (
		<div className='fixed inset-0 z-50 flex items-center justify-center p-4 transition-all'>
			...
			<div className='relative z-20 w-full max-w-2xl bg-white dark:bg-gray-900 rounded-xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh] font-display text-gray-800 dark:text-gray-100 animate-in fade-in zoom-in duration-200'>
				{/* Header */}
				<header className='px-8 py-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center bg-white dark:bg-gray-900 sticky top-0 z-30'>
					...
					<button
						onClick={onClose}
						className='w-10 h-10 flex items-center justify-center rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors text-gray-500 dark:text-gray-400 cursor-pointer'
				...
				<div className='flex-1 overflow-y-auto p-8 space-y-10 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]'>
					...
					<section>
						...
						<div className='relative h-12 flex items-center mb-6 px-3 group'>
							<div className='absolute left-3 right-3 h-1 bg-gray-200 dark:bg-gray-700 rounded-full'>
								<div
							...
							<div className='flex justify-between items-center'>
								...
								<div className='flex items-center space-x-3 bg-[#f5f8f6] dark:bg-gray-800 rounded-full p-1'>
									<button
										...
										className='w-8 h-8 rounded-full bg-white dark:bg-gray-700 shadow-sm flex items-center justify-center text-gray-500 hover:text-[#006611] transition-colors active:scale-90'
									>
									...
									<button
										...
										className='w-8 h-8 rounded-full bg-white dark:bg-gray-700 shadow-sm flex items-center justify-center text-[#006611] hover:bg-[#006611] hover:text-white transition-colors active:scale-90'
									>
										...
										<div
											className={`h-full px-4 py-3 rounded-lg border transition-all flex items-center justify-center gap-2 ${
												amenities.includes(item.id)
													? 'border-[#006611] bg-[#006611]/5 dark:bg-[#006611]/20 text-[#006611]'
